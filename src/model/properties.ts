...
export interface ScalarProperty {
  source?:
    | { $case: "constValue"; constValue: number }
...
export interface VectorProperty {
  source?:
    | { $case: "constValue"; constValue: Vector }
    | { $case: "pathId"; pathId: number };
}
...
export const ScalarProperty = {
  ...
  fromPartial<I extends Exact<DeepPartial<ScalarProperty>, I>>(
    object: I
  ): ScalarProperty {
    const message = createBaseScalarProperty();
    if (
      ...
      message.source = {
        $case: "constValue",
        constValue: object.source.constValue,
      };
    }
    if (
      object.source?.$case === "graphId" &&
      object.source?.graphId !== undefined &&
...
export const VectorProperty = {
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        case 1:
          message.source = {
            $case: "constValue",
            constValue: Vector.decode(reader, reader.uint32()),
          };
          break;
        case 2:
  ...
  fromJSON(object: any): VectorProperty {
    return {
      source: isSet(object.constValue)
        ? {
            $case: "constValue",
            constValue: Vector.fromJSON(object.constValue),
          }
        : isSet(object.pathId)
  ...
  toJSON(message: VectorProperty): unknown {
    ...
    message.source?.$case === "constValue" &&
      (obj.constValue = message.source?.constValue
        ? Vector.toJSON(message.source?.constValue)
        : undefined);
    message.source?.$case === "pathId" &&
      (obj.pathId = Math.round(message.source?.pathId));
    return obj;
  },

  fromPartial<I extends Exact<DeepPartial<VectorProperty>, I>>(
    object: I
  ): VectorProperty {
    const message = createBaseVectorProperty();
    if (
      ...
      message.source = {
        $case: "constValue",
        constValue: Vector.fromPartial(object.source.constValue),
      };
    }
    if (
      object.source?.$case === "pathId" &&
      object.source?.pathId !== undefined &&
      object.source?.pathId !== null
    ) {
...
export const ColorProperty = {
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        case 1:
          message.source = {
            $case: "constValue",
            constValue: Color.decode(reader, reader.uint32()),
          };
          break;
        case 2:
          message.source = { $case: "graphId", graphId: reader.uint32() };
  ...
  toJSON(message: ColorProperty): unknown {
    ...
    message.source?.$case === "constValue" &&
      (obj.constValue = message.source?.constValue
        ? Color.toJSON(message.source?.constValue)
        : undefined);
    message.source?.$case === "graphId" &&
      (obj.graphId = Math.round(message.source?.graphId));
  ...
  fromPartial<I extends Exact<DeepPartial<ColorProperty>, I>>(
    object: I
  ): ColorProperty {
    const message = createBaseColorProperty();
    if (
      ...
      message.source = {
        $case: "constValue",
        constValue: Color.fromPartial(object.source.constValue),
      };
    }
    if (
      object.source?.$case === "graphId" &&
